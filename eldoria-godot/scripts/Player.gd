...
func _ready() -> void:
	...
	call_deferred("_normalize_player_model", 1.8)  # scale-eng 2026-05-08: clamped 1.1 → 1.8 (canon floor 1.6m, Player/Hero NPC range 1.6–2.0m)
...
func _physics_process(delta: float) -> void:
	...
	# 2026-05-06: Standard Godot forward = -Z. Model rotation fix in Main.tscn now matches.
	...
	if Input.is_key_pressed(KEY_BRACKETRIGHT) or Input.is_key_pressed(KEY_BACKSPACE):
		...
		is_attacking = false
		if hp <= 0: hp = max_hp
		# 2026-05-06: force-snap to ground after teleport so we don't get re-stuck in air
		call_deferred("_snap_to_ground", 50.0)
		return
	...
	if global_position.y < -50.0 or global_position.y > 500.0:
		global_position = Vector3(15, 10, 15)  # PX SAFE_SPAWN — Y=3→5 2026-05-06 (avoid below-ground spawn) — clear of village
		velocity = Vector3.ZERO
...
func _respawn_at_well() -> void:
	...
	global_position = Vector3(15, 10, 15)  # PX SAFE_SPAWN — Y=3→5 2026-05-06 (avoid below-ground spawn) — was inside well
	hp = max_hp
	mp = max_mp
	is_dead = false
	stats_changed.emit()
...
func _schedule_normalize_retry(delay: float) -> void:
	...
	if is_instance_valid(self):
		_normalize_player_model(1.8)  # scale-eng 2026-05-08: clamped 1.1 → 1.8 (canon floor 1.6m)
...
func _snap_to_ground(max_drop: float = 50.0) -> void:  # 2026-05-06 4→50 to handle large terrain dips
	# Below-ground rescue: if we somehow fell beneath the world (Y < -2),
	# teleport up to safe spawn Y. Without this, the snap raycast below
	# might miss because we're already below all collidable geometry.
